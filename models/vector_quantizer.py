...
def sample_vectors(samples, num):
    ...
    if num_samples >= num:
        indices = torch.randperm(num_samples)[:num].to(device)
    else:
        indices = torch.randint(0, num_samples, (num,)).to(device)
...
class EuclideanCodebook(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            ema_inplace(self.embed_avg, embed_sum, self.decay) # new
    ...
    def forward(self, x):
        ...
        if self.training:
            if self.commitment_weight > 0:
                ...
                # commit_loss = F.mse_loss(quantize.detach(), x, reduction="sum")
