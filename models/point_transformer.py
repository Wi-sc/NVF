...
import sys
from models.lib.pointops.functions import pointops
...
class PointTransformerSeg(nn.Module):
    ...
    def forward(self, p0):
        batch_size, point_number = p0.shape[:2]
        o0 = torch.arange(1, batch_size+1, dtype=torch.int).to(p0.device)
        o0 *= point_number
        p0 = p0.view(batch_size*point_number, 3)
        x0 = p0
        ...
        x = self.cls(x1)
        return x.view(batch_size, point_number, -1)
...
if __name__=="__main__":
    ...
    points = torch.rand(2, 10000, 3).to(device)
    model = pointtransformer_seg_repro(c=3).to(device)
    # prediction = model([points.view(-1, 3), None, torch.IntTensor([10000,20000]).to(device)])
